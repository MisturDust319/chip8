    ...
    : randGen(std::chrono::system_clock::now().time_since_epoch().count())
{
    ...
    // fill the function pointer tables
...
void Chip8::Table0()
{
    // all opcodes executed by Table0 begin with 00E
    // so the final nibble is used to determine the opcode to execute
    ((*this).*(table0[opcode & 0x000Fu]))();
}

void Chip8::Table8()
{
    // the opcodes that Table8 derefs all begin with 8xy
    // it is the final nibble that determines what operation to execute
    ((*this).*(table8[opcode & 0x000Fu]))();
}

void Chip8::TableE()
{
    // techincally the last two nibbles in instructions beggining with E are unique
    // but there are only two instructions, each with a unique final nibble
    // so we use that to index the operations
    ((*this).*(tableE[opcode & 0x000Fu]))();
}

void Chip8::TableF()
{
    // The first two nibbles in opcodes beginning with F are Fx
    // so we use the last TWO nibbles to index the proper operation
